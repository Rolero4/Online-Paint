...
function drawFromPhp() {
  paintings.forEach((element) => {
    if (element.name === "Curve") drawCurve(element, contextFinal);
    else if (element.name === "Line") drawLine(element, contextFinal);
    else if (element.name === "Circle") drawCircle(element, contextFinal);
  });
}

function touchStart(e) {
  if (paintingStyle === "brush") brushStart(e.touches[0]);
  if (paintingStyle === "line") lineStart(e);
  if (paintingStyle === "circle") circleStart(e);
}
function touchMove(e) {
  if (paintingStyle === "brush") brushMove(e.touches[0], e.preventDefault());
  if (paintingStyle === "line") lineMove(e, e.preventDefault());
  if (paintingStyle === "circle") circleMove(e, e.preventDefault());
}
function touchEnd(e) {
  if (paintingStyle === "brush") brushEnd(e.changedTouches[0]);
  if (paintingStyle === "line") lineEnd(e);
  if (paintingStyle === "circle") circleEnd(e);
}
//#endregion
...
function brushStart(event) {
  ...
  contextDraft.moveTo(
    currentBrushPoint.x * canvasDraft.width,
    currentBrushPoint.y * canvasDraft.height
  );
}
...
function drawCurve(Curve, context) {
  if (Curve.brushPoints.length > 1) {
    context.beginPath();
    context.moveTo(
      Curve.brushPoints[0].x * canvasDraft.width,
      Curve.brushPoints[0].y * canvasDraft.height
    );
    context.strokeStyle = Curve.color;
    context.lineWidth = Curve.width;
    for (let i = 0; i < Curve.brushPoints.length; i++) {
      context.lineTo(
        Curve.brushPoints[i].x * canvasDraft.width,
        Curve.brushPoints[i].y * canvasDraft.height
      );
      context.stroke();
    }
    context.closePath();
...
function drawLine(line, context) {
  context.beginPath();
  context.moveTo(
    line.startPoint.x * canvasDraft.width,
    line.startPoint.y * canvasDraft.height
  );
  context.lineTo(
    line.endPoint.x * canvasDraft.width,
    line.endPoint.y * canvasDraft.height
  );
...
function drawCircle(circle, context) {
  ...
  let radius =
    Math.hypot(
      circle.startPoint.x * canvasDraft.width -
        circle.endPoint.x * canvasDraft.width,
      circle.startPoint.y * canvasDraft.height -
        circle.endPoint.y * canvasDraft.height
    ) / 2;
  let centerPoint = {
    x:
      (circle.startPoint.x * canvasDraft.width +
      ...
      if (xhr.status === 200 || xhr.status === 304) {
        try {
          ...
          if (JSON.stringify(paintings) != this.response) {
            clearCanvas(contextFinal);
            paintings.length = 0;
            readyResponse.forEach((element) => paintings.push(element));
            drawFromPhp();
          }
        } catch (error) {}
      }
